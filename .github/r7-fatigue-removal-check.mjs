...
const expectedHuman = ["protan", "deutan", "tritan", "blur", "low_contrast", "cataract", "tunnel", "central_loss", "night", "dry_eye"];
const expectedSpatial = ["Normal", "Tunnel Vision", "Central Loss", "Night / Low Light", "Dog-like", "Cataract-like"];
...
async function noOverflow(page, label) {
  ...
  assert(metrics.doc <= metrics.inner + 1 && metrics.body <= metrics.inner + 1, `${label}: overflow ${JSON.stringify(metrics)}`);
}
...
try {
  for (const viewport of [{ width: 1440, height: 1000 }, { width: 390, height: 844 }]) {
    ...
    const human = await page.locator("#mode-select option").evaluateAll((nodes) => nodes.map((node) => node.value));
    ...
    const animal = await page.locator("#mode-select option").evaluateAll((nodes) => nodes.map((node) => node.value));
    ...
    const references = await page.locator("#mode-select option").evaluateAll((nodes) => nodes.map((node) => node.value));
  ...
  for (const viewport of [{ width: 1440, height: 1000 }, { width: 390, height: 844 }]) {
    ...
    const labels = await page.getByRole("group", { name: "Spatial perception mode" }).getByRole("button").allTextContents();
