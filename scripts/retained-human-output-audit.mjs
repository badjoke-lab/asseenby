...
for (const mode of modes) {
  ...
  const beforeMode = await approx.getAttribute('src');
  await page.locator('#mode-select').selectOption(mode);
  if (await approx.getAttribute('src') === beforeMode) {
    await page.waitForFunction((prev) => document.querySelector('img[alt="Approximation"]')?.getAttribute('src') !== prev, beforeMode, { timeout: 10000 }).catch(() => {});
  }
  await page.locator('.compare-card[aria-busy="false"]').waitFor({ timeout: 10000 });

  for (const strength of strengths) {
    const before = await approx.getAttribute('src');
    await page.locator('#strength-range').evaluate((el, value) => {
    ...
    await page.waitForFunction((prev) => {
      ...
      return img?.getAttribute('src')?.startsWith('blob:') && img.getAttribute('src') !== prev && document.querySelector('.compare-card')?.getAttribute('aria-busy') === 'false';
    }, before, { timeout: 10000 });

    const metrics = await page.evaluate(async () => {
      const orig = document.querySelector('img[alt="Original"]');
      const approx = document.querySelector('img[alt="Approximation"]');
      if (!orig || !approx) throw new Error('missing images');
      await Promise.all([orig.decode(), approx.decode()]);
      ...
      x1.drawImage(orig,0,0,w,h); x2.drawImage(approx,0,0,w,h);
