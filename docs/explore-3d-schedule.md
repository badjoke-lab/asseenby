...
Astra deployment constraint verified against Cloudflare Pages limits: each file must be under 25 MiB. The b6ff303 candidate is 31,366,728 bytes. Lossless GLB packing removes unused UV/tangent attributes from untextured components while asserting byte-identical positions, normals, indices and images; tested result 26,097,020 bytes. This is a hosting correction, not an inspector-cache workaround or an architecture reduction.
